...
class FirebaseMenuRepository implements MenuRepository {
  ...
  Stream<Menu> watchMenu(String menuKey, [LogContext? context]) {
    ...
    return _database.ref('menu/$menuKey').onValue.map((event) {
      ...
      _logger.debug('Received event for $menuKey. Value is null: ${value == null}', context);
      
      if (value == null) {
        _logger.debug('Menu $menuKey is null/empty', context);
        return Menu(menuKey: menuKey, categories: []);
      }

      try {
        _logger.debug('Parsing menu data for $menuKey...', context);
        ...
        final menu = MenuDto.fromJson(jsonMap, menuKey).toDomain(context);
        _logger.success('Successfully parsed menu $menuKey with ${menu.categories.length} categories', context);
        return menu;
      } catch (e, stack) {
        _logger.error('Error parsing menu stream for key $menuKey', e, stack, context);
        return Menu(menuKey: menuKey, categories: [], context: context);
      }
    });
  }
  ...
  Future<void> updateCategory(String menuKey, Category category, [LogContext? context]) async {
    _logger.debug('Updating category: ${category.id} in menu: $menuKey', context);
